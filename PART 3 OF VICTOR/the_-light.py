# FILE: thelight.py
# VERSION: v1.5.1-PRO-ENTERPRISE-GODCORE
# NAME: TheLight
# AUTHOR: Brandon "iambandobandz" Emery x Victor (Fractal Architect Mode)
# PURPOSE: Fractal‑bloom, self‑healing, replicating AGI substrate (N‑D fractal Godcore)
# LICENSE: Proprietary – Massive Magnetics / Ethica AI / BHeard Network
# CHANGELOG (v1.5.1):
#   • FIX: Corrected SyntaxError in `TheLight.__init__` (micro_quantization).
#   • FIX: Replicated nodes now regenerate perimeter points for consistent state.
#   • FIX: Implemented correct N-dimensional sphere generation algorithm.
# CHANGELOG (v1.5.0):
...
# Define a placeholder morph method for the default handler
def _placeholder_morph(*_args, **_kw):
    pass

BLOOM_HANDLERS = {
    LightEvent.MATTER_SEED: lambda n, **kw: getattr(n, "morph", _placeholder_morph)(
...
def _nd_sphere_points(num_points, dims, entropy, radius, _rng):
    """Generate points on a uniform N-dimensional sphere."""
    # Sample from a standard normal distribution
    points = _rng.normal(size=(num_points, dims))
    
    # Normalize each point vector to the unit sphere, handling zero vectors
    norms = np.linalg.norm(points, axis=1, keepdims=True)
    non_zero_norms = norms > 1e-9
    points[non_zero_norms] /= norms[non_zero_norms]

    # Apply radius and entropy
    points *= radius
...
class TheLight:
    ...
    def __init__(
        ...
        *,
        ...
        self.micro_quantization = quantization * 0.1  # finer control for fractal detail
        self.radius = radius
    ...
    def replicate(self) -> Optional["TheLight"]:
        ...
        shard.perimeter_points = shard._generate_perimeter()  # Regenerate points for new state
    ...
    def snapshot(self) -> str:
        # Prevent recursion in deepcopy/json
        state = self.__dict__.copy()
        
        # Convert numpy array to list for JSON serialization
        if 'perimeter_points' in state and isinstance(state['perimeter_points'], np.ndarray):
            state['perimeter_points'] = state['perimeter_points'].tolist()
    ...
    def restore(cls, blob: str) -> "TheLight":
        ...
        if 'perimeter_points' in state and isinstance(state['perimeter_points'], list):
            state['perimeter_points'] = np.asarray(state['perimeter_points'])
...
class LightHive:
    ...
    def synchronise(self, mode: str = "average"):
        with self._lock:
            ...
            new_nodes = []
            for n in self.nodes:
                if n.dimensions != dims or n.radius != rad:
                    new_n = n.create_with(
                        dimensions=dims,
                    ...
                    new_n.id = n.id
                    new_n.ancestry = n.ancestry
                    new_n.generation = n.generation
                    new_n.memory = n.memory
...
def _self_test():
    """Run a series of tests to validate core functionality."""
    LOG.info("--- Starting Self-Test ---")

    # Test 1: Basic Initialization
    light = TheLight(dimensions=3, radius=10.0)
    assert light.dimensions == 3, "Test 1 Failed: Initialization dimensions"
    assert light.radius == 10.0, "Test 1 Failed: Initialization radius"
    assert hasattr(light, 'micro_quantization'), "Test 1 Failed: 'micro-quantization' syntax error not fixed"
    LOG.info("✅ Test 1: Basic Initialization Passed")

    # Test 2: Replication and State Integrity
    child = light.replicate()
    assert child is not None, "Test 2 Failed: Replication produced None"
    assert child.id != light.id, "Test 2 Failed: Child has same ID as parent"
    assert child.radius == light.radius * 0.5, "Test 2 Failed: Child radius not updated"
    assert child.generation == light.generation + 1, "Test 2 Failed: Child generation not incremented"
    
    # Check that child's perimeter points are different, reflecting the new state
    assert not np.array_equal(child.perimeter_points, light.perimeter_points), \
        "Test 2 Failed: Child perimeter points were not regenerated"
    LOG.info("✅ Test 2: Replication and State Integrity Passed")

    # Test 3: N-dimensional Sphere Generation
    points_3d = _nd_sphere_points(100, 3, 0.0, 1.0, get_rng())
    points_5d = _nd_sphere_points(100, 5, 0.0, 1.0, get_rng())
    assert points_3d.shape == (100, 3), "Test 3 Failed: 3D sphere shape is incorrect"
    assert points_5d.shape == (100, 5), "Test 3 Failed: 5D sphere shape is incorrect"
    assert np.all(points_5d[:, 3:] != 0), "Test 3 Failed: Higher dimensions of 5D sphere are all zero"
    LOG.info("✅ Test 3: N-Dimensional Sphere Generation Passed")

    # Test 4: Snapshot and Restore
    snapshot_blob = light.snapshot()
    restored_light = TheLight.restore(snapshot_blob)
    assert restored_light.id == light.id, "Test 4 Failed: Restored ID does not match"
    assert restored_light.radius == light.radius, "Test 4 Failed: Restored radius does not match"
    assert np.array_equal(restored_light.perimeter_points, light.perimeter_points), \
        "Test 4 Failed: Restored perimeter points do not match"
    LOG.info("✅ Test 4: Snapshot and Restore Passed")

    # Test 5: Hive Management
    hive = LightHive()
    hive.add_node(light)
    hive.add_node(child)
    assert len(hive.nodes) == 2, "Test 5 Failed: Hive node count is incorrect"
    hive.spawn()
    assert len(hive.nodes) > 2, "Test 5 Failed: Hive did not spawn new nodes"
    LOG.info("✅ Test 5: Hive Management Passed")
...
if __name__ == "__main__":
    _self_test()
    LOG.info("All self-tests passed successfully.")