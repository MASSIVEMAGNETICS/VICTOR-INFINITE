# ================================================================================================= 
# ||    SOUNDclick BOT GODMODE v3.1 - GHOST PROTOCOL                                                  ||
# ||    "The best operator is the one they never knew was there."                               ||
# ||    SoundClick auto-song loader baked in!                                                   ||
# =================================================================================================
...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QMessageBox, QFileDialog, QListWidget, QListWidgetItem, QCheckBox, QSizePolicy)
...
class SoundBotGUI(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Artist Song Scraper Row
        artist_row = QHBoxLayout()
        artist_row.addWidget(QLabel("SoundClick Artist URL:"))
        self.artist_url_input = QLineEdit()
        self.artist_url_input.setPlaceholderText("https://www.soundclick.com/artist/your-artist-name")
        artist_row.addWidget(self.artist_url_input)
        self.load_songs_btn = QPushButton("Load Songs")
        artist_row.addWidget(self.load_songs_btn)
        main_layout.addLayout(artist_row)

        self.song_list_widget = QListWidget()
        self.song_list_widget.setSelectionMode(QListWidget.NoSelection)
        self.song_list_widget.setMaximumHeight(200)
    ...
    def _on_start(self):
        # Step 1: Get checked songs from list
        checked_urls = []
        ...
        # Step 2: Add manual URLs if entered
        manual_urls = [url.strip() for url in self.url_input.toPlainText().splitlines() if url.strip()]
        all_urls = list(set(checked_urls + manual_urls))
        if not all_urls or not self.proxy_api_input.text() or not self.selector_input.text():
            QMessageBox.critical(self, "Error", "You must select at least one song or enter a URL, and Proxy API/Selector is required."); return

        self.play_log_data.clear(); self._log("[INFO] Log cache cleared for new run.")
        config = {'urls': all_urls, 'proxy_api_url': self.proxy_api_input.text(), 'selector': self.selector_input.text(),
    ...
    def _on_load_songs(self):
        artist_url = self.artist_url_input.text().strip()
        if not artist_url:
            QMessageBox.warning(self, "Missing URL", "Enter a SoundClick artist URL.")
            return
        self.song_list_widget.clear()
        self._log(f"[INFO] Fetching songs from: {artist_url}")
        try:
            resp = requests.get(artist_url, timeout=10)
            if resp.status_code != 200:
                raise Exception(f"Status code {resp.status_code}")
            soup = BeautifulSoup(resp.text, "html.parser")
            found_any = False
            # Typical SoundClick: song links have class="song-title" or similar
            for a in soup.find_all("a", href=True):
                href = a["href"]
                # Only real song links
                if "/music/songInfo.cfm" in href or "/artist/sng" in href:
                    # Absolute or relative link
                    if href.startswith("http"):
                        url = href
                    else:
                        url = "https://www.soundclick.com" + href
                    ...
                    found_any = True
            if not found_any:
                self._log("[WARN] No songs found. Try a different artist URL or check site structure.")
                QMessageBox.information(self, "No Songs Found", "Could not find any songs on this page.")
            else:
                self._log(f"[INFO] {self.song_list_widget.count()} songs loaded. Uncheck any you want to skip.")
        except Exception as e:
            self._log(f"[ERROR] Could not load songs: {e}")
