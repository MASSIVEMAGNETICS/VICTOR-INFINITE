# =================================================================================================
# ||    SOUNDBOT GODMODE v3.1 - Ghost Protocol (Auto Artist Song Scraper, Fully Baked-In)         ||
# =================================================================================================
...
class BrowserEngine:
    @staticmethod
    def get_stealth_browser(proxy=None, geo=None):
        fake = Faker()
    ...
    @staticmethod
    def scrape_artist_song_urls(artist_url):
        """ Uses Selenium stealth browser to scrape all song URLs from a SoundClick artist page. """
        browser = None
        try:
            browser = BrowserEngine.get_stealth_browser()
            browser.get(artist_url)
            time.sleep(7)  # Allow JS to fully render
            # This selector might need tweaking based on SoundClick's DOM!
            links = browser.find_elements(By.CSS_SELECTOR, 'a[href*="/music/song/"]')
            song_urls = []
            for link in links:
                url = link.get_attribute("href")
                if url and url not in song_urls:
                    song_urls.append(url)
            if not song_urls:
                raise ValueError("No song URLs found. Check selector or if artist page is valid.")
            return song_urls
        except Exception as e:
            logging.error(f"Failed to scrape songs: {e}")
            raise
        finally:
...
class SoundBotSwarm(QObject):
    ...
    def _send_webhook(self, message, success=True):
        ...
        try:
            ...
            requests.post(self.config['webhook_url'], json={"embeds": [{"description": message, "color": color, "footer": {"text": f"SOUNDBOT GODMODE v3.1"}}]}, timeout=5)
...
class SoundBotGUI(QMainWindow):
    ...
    def _setup_menu(self):
        ...
        save_songs_action = file_menu.addAction("Save Song URL List…")
        save_songs_action.triggered.connect(self._save_song_url_list)

        save_proxy_action = file_menu.addAction("Save Proxy API URL…")
        save_proxy_action.triggered.connect(self._save_proxy_url)

        file_menu.addSeparator()
        exit_action = file_menu.addAction("Exit")
        exit_action.triggered.connect(self.close)

    def _save_song_url_list(self):
        urls = self.url_input.toPlainText()
        if not urls.strip():
            QMessageBox.warning(self, "Save Error", "No song URLs to save.")
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Song URLs", "song_urls.txt", "Text Files (*.txt)")
        if path:
            try:
                with open(path, "w", encoding="utf-8") as f:
                    f.write(urls)
                self._log(f"[INFO] Song URL list saved to {path}")
            except Exception as e:
                QMessageBox.critical(self, "Save Error", f"Failed to save song URLs:\n{e}")

    def _save_proxy_url(self):
        proxy_url = self.proxy_api_input.text()
        if not proxy_url.strip():
            QMessageBox.warning(self, "Save Error", "No proxy API URL to save.")
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Proxy API URL", "proxy_api_url.txt", "Text Files (*.txt)")
        if path:
            try:
                with open(path, "w", encoding="utf-8") as f:
                    f.write(proxy_url)
                self._log(f"[INFO] Proxy API URL saved to {path}")
            except Exception as e:
                QMessageBox.critical(self, "Save Error", f"Failed to save proxy API URL:\n{e}")
    ...
    def _setup_ui(self):
        ...
        artist_row.addWidget(QLabel("SoundClick Artist URL:"))
        self.artist_url_input = QLineEdit()
        self.artist_url_input.setPlaceholderText("https://www.soundclick.com/your-artist-url")
        artist_row.addWidget(self.artist_url_input)
        self.scrape_btn = QPushButton("Load Songs")
        self.scrape_btn.clicked.connect(self._on_scrape_songs)
        artist_row.addWidget(self.scrape_btn)
        main_layout.addLayout(artist_row)
        # === SONG URL LIST ===
        main_layout.addWidget(QLabel("🎵 Select songs to target:"))
        self.songlist_box = QTextEdit()
        self.songlist_box.setPlaceholderText("List of scraped or manual song URLs will appear here.")
        main_layout.addWidget(self.songlist_box)
        main_layout.addWidget(QLabel("🛑 Or paste Target URLs manually (one per line):"))
        self.url_input = QTextEdit()
        self.url_input.setPlaceholderText("https://www.soundclick.com/artist/your-song-1\n....")
    ...
    def _on_scrape_songs(self):
        url = self.artist_url_input.text().strip()
        if not url.startswith("http"):
            QMessageBox.critical(self, "Scrape Error", "Enter a valid SoundClick artist URL.")
            return
        try:
            self._log(f"Scraping songs from {url} ...")
            song_urls = BrowserEngine.scrape_artist_song_urls(url)
            if not song_urls:
                QMessageBox.critical(self, "Scrape Error", "No songs found on artist page. Check selector or URL.")
                return
            self.songlist_box.setPlainText('\n'.join(song_urls))
            self.url_input.setPlainText('\n'.join(song_urls))
            self._log(f"[INFO] {len(song_urls)} songs loaded.")
        except Exception as e:
            QMessageBox.critical(self, "Scrape Error", f"Failed to fetch/parse artist page:\n{e}")

    def _on_start(self):
        # Use song list box if populated, else manual input
        urls = []
        url_text = self.songlist_box.toPlainText().strip()
        if url_text:
