...
def _compute_global_attention_mask(input_ids, sep_token_id, before_sep_token=True):
    """
    Computes the global attention mask for the input_ids based on the `sep_token_id`.
    If `before_sep_token` is True, global attention is set on all tokens before the first `sep_token_id`.
    Otherwise, it is set on all tokens after the first `sep_token_id`.
    """
    ...
    if sep_indices.numel() > 0:
        if before_sep_token:
            global_attention_mask[:, : sep_indices[0, 1]] = 1
        else:
            global_attention_mask[:, sep_indices[0, 1] + 1 :] = 1

    return global_attention_mask.unsqueeze(0).unsqueeze(0)