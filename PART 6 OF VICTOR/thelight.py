    ...
    *,
    ...
    if kind == "sphere":  # ND Fibonacci‑sphere
        ...
        for i in range(num_points):
            ...
            coords = [v + rng.normal(0, entropy * radius * 0.1) for v in coords]
            pts.append(coords)
        return nan_guard(np.asarray(pts))
...
class TheLight:
    ...
    def _generate_perimeter(self):
        ...
        # fallback sphere
        return fractal_perimeter(num_pts, kind="sphere", dims=self.dimensions, entropy=self.entropy, radius=self.radius)
